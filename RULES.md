...
## 🧱 Baseline יציב לצ'אט (לא לשבור)

כל עבודת AI בצ'אט חייבת לשמור קודם על הבסיס הבא:

- `apps/web/app/api/v1/ai/chat/route.ts`
  - `export const runtime = 'edge'`
  - `streamText` + `toTextStreamResponse`
  - ספק צ'אט יחיד: `openai/gpt-5-mini` דרך OpenRouter
- `apps/web/components/ai/AIChatWidget.tsx`
  - `useChat` + `TextStreamChatTransport`

DeepSeek מיועד לעבודות רקע (analysis/cron), לא לצ'אט בזמן אמת, אלא אם יש החלטת מוצר מפורשת.

## 🧪 שינויים בצעדים קטנים + rollback מהיר

חובה לעבוד כך על צ'אט:

1. שינוי אחד קטן בכל קומיט.
2. בדיקת smoke ידנית אחרי כל שינוי (3 הודעות רצופות לפחות).
3. רק אם יציב — להמשיך לשינוי הבא.
4. אם נשבר — rollback מיידי עם `git revert` לקומיט הפוגע.
...
אין לבצע קפיצות ארכיטקטוניות גדולות בצ'אט בלי עוגן ביניים עובד ומאומת.
