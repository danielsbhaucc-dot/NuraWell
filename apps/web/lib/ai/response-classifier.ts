 ...
 */
const FAILED_RE =
  /(?:ניסיתי(?:\s+אבל)?\s+(?:לא\s+(?:הצלחתי|הספקתי)|לא\s+יצא)|השתדלתי(?:\s+אבל)?\s+לא|כמעט\s+הצלחתי\s+אבל|התחלתי\s+אבל\s+(?:לא\s+סיימתי|לא\s+יכולתי|נשבר)|שכחתי\s+(?:לשתות|לעשות|לבצע|את\s+ה)|לא\s+הספקתי\s+אבל\s+ניסיתי|לא\s+הצלחתי\s+(?:להגיע|לעשות|לבצע)|נשבר\s+לי|פאשלתי|כשלון|רציתי\s+אבל\s+לא\s+(?:יצא|הצלחתי)|נפלתי\s+אבל)/i;
