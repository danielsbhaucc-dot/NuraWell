...
export function formatDailyShortTermBlock(input: DailyBlockInput): string | null {
  ...
  return `[יום] ${segs.join(' · ')} — התאם טון ומנע חזרתיות בלבד; מגעים קודמים אינם הוראת פעולה. אם המשתמש כתב רק ברכה/הודעה כללית, אל תמשיך אוטומטית מהתראה קודמת ואל תציע "מתחילים מחדש" בלי בקשה מפורשת. אירוע/לוויה/מילואים=בלי תזכורת גנרית.`;
}
