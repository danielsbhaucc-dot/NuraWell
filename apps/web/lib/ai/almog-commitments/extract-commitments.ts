...
- אם אתה לא בטוח שאלמוג אמר את זה — תן confidence נמוך (מתחת ל-0.6) או אל תכלול.
- אל תהפוך אמירה כללית ("כדאי לשתות מים") למשימה. משימה = אלמוג נתן הוראה ספציפית שהמשתמש אמור לעשות.
- תזכורת = אלמוג אמר במפורש שיזכיר/נתזכר ("אני אזכיר לך", "נתראה בערב על זה").
...
function normalize(parsed: Record<string, unknown>, now: Date): CommitmentExtraction {
  ...
  if (Array.isArray(parsed.tasks)) {
    out.tasks = (parsed.tasks as unknown[])
      ...
      .filter((x): x is ExtractedTask => x !== null && x.confidence >= MIN_CONFIDENCE)
      .slice(0, 4);
  }
