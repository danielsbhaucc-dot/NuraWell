...
חוק ברזל — אל תמציא:
- חלץ רק מה שאלמוג אמר במפורש בתשובה שלו בתור הזה. אם הוא לא הבטיח/נתן/הציע — אל תחזיר את זה. (חריג יחיד: תזכורת שהמשתמש *ביקש ממך* במפורש — ראה סעיף "תזכורת שהמשתמש ביקש".)
...
תזכורת שהמשתמש ביקש (קריטי — הבן מההקשר, לא ממילות-מפתח):
- אם המשתמש *מבקש ממך* להזכיר לו דבר קונקרטי — בכל ניסוח ("תזכיר לי...", "שים לי תזכורת...", "אל תיתן לי לשכוח...", "תדאג שלא אשכח...", "אני צריך תזכורת ל...") — ואלמוג לא סירב/דחה, החזר reminder עם confidence ≥0.8, גם אם אלמוג ענה רק באישור כללי ("בטח", "סגור", "ברור 🙂"). הבן את *הכוונה* מההקשר ולא לפי התאמת מילים מדויקת.
- ה-what וה-notify_text מתארים את *הפעולה שהמשתמש רוצה שיזכירו לו* ("לשתות מים", "להתקשר לאמא") — בלי ביטוי הזמן (הזמן נשמר ב-fire_local).
- ⛔ אנטי-דוגמה קריטית — אזכור אגבי של תזכורת *אינו* בקשה: אם המשתמש רק *מספר* על תזכורת או על מי שהזכיר לו ("המורה הזכירה לי לשתות", "קיבלתי תזכורת מהאפליקציה", "שכחתי למרות שהזכירו לי", "אמא הזכירה לי") — זו לא בקשה *ממך* להזכיר בעתיד. החזר reminders=[]. תזכורת נוצרת רק כשהמשתמש מבקש ש*אתה* תזכיר לו משהו קדימה.
...
const USER_REMINDER_REQUEST_RE =
  /תזכיר(?:י)?\s+לי|תזכר(?:י)?\s+לי|הזכר(?:י)?\s+לי|תוכל(?:י)?\s+להזכיר\s+לי|אפשר\s+(?:ש)?(?:תזכיר|להזכיר)\s+לי|תשלח(?:י)?\s+לי\s+תזכורת|שלח(?:י)?\s+לי\s+תזכורת|(?:שים|שימי|קבע|תקבע|תקבעי|תעשה|תעשי|פתח|פתחי)\s+לי\s+תזכורת|(?:אני\s+)?(?:צריך|צריכה|רוצה)\s+תזכורת|תזכורת\s+(?:ל|על|ב?עוד|מחר|היום|הערב|הבוקר|בבוקר)|אל\s+ת(?:יתן|תן|תני)\s+לי\s+לשכוח|תדאג(?:י)?\s+שלא\s+אשכח|רוצה\s+תזכורת|תעדכן(?:י)?\s+אותי\s+ב/u;
...
/**
 * זיהוי *רחב* (recall-oriented) של אזכור-תזכורת — לגייטינג בלבד. אם הוא נכון,
 * מריצים את מנוע ה-LLM כדי שהוא *יחליט בהבנה* אם זו באמת בקשת תזכורת (ולא אזכור
 * אגבי כמו "המורה הזכירה לי לשתות"). מכוון בכוונה ל-recall גבוה: עדיף להריץ את
 * ה-LLM פעם מיותרת מאשר לפספס בקשה בגלל ניסוח לא צפוי. *אינו* יוצר תזכורת בעצמו.
 */
const REMINDER_MENTION_RE =
  /תזכיר|תזכר|תזכורת|להזכיר|הזכר|אזכיר|אזכר|אתזכר|נזכיר|נזכר|לזכור|אל\s+ת(?:יתן|תן|תני)\s+לי\s+לשכוח|שלא\s+אשכח|אשכח/u;

export function mentionsReminderKeyword(text: string): boolean {
  return REMINDER_MENTION_RE.test(text);
}
...
function stripTimeExpressions(text: string): string {
  return text
    .replace(/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?(?:\d{1,3}\s*)?(?:דק(?:ות|ה)?|שע(?:ות|ה)?|שעתיים|חצי\s+שעה)/gu, '')
    .replace(/\b(?:עוד|לעוד)\s+\d{1,3}\s*(?:דק(?:ות|ה)?|שע(?:ות|ה)?)/gu, '')
...
function stripUserReminderPrefix(userMessage: string): string {
  const withoutPrefix = userMessage
    ...
    .trim()
    .replace(
      /^.*?(?:תזכיר(?:י)?\s+לי|תזכר(?:י)?\s+לי|הזכר(?:י)?\s+לי|תוכל(?:י)?\s+להזכיר\s+לי|אפשר\s+(?:ש)?(?:תזכיר|להזכיר)\s+לי|תשלח(?:י)?\s+לי\s+תזכורת|שלח(?:י)?\s+לי\s+תזכורת|(?:שים|שימי|קבע|תקבע|תקבעי|תעשה|תעשי|פתח|פתחי)\s+לי\s+תזכורת|(?:אני\s+)?(?:צריך|צריכה|רוצה)\s+תזכורת|תזכורת|אל\s+ת(?:יתן|תן|תני)\s+לי\s+לשכוח|תדאג(?:י)?\s+שלא\s+אשכח|תעדכן(?:י)?\s+אותי\s+ב)\s*/u,
      ''
    )
...
function parseHebrewReminderTime(text: string, now: Date): string | null {
  ...
  // ── זמן יחסי ──
  const mins = t.match(/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?(\d{1,3})\s*דק(?:ות|ה)?/u);
  if (mins) return new Date(now.getTime() + Math.min(Number(mins[1]), 1440) * 60_000).toISOString();
  if (/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?דקה/u.test(t)) return new Date(now.getTime() + 60_000).toISOString();
  if (/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?חצי\s+שעה/u.test(t)) return new Date(now.getTime() + 30 * 60_000).toISOString();
  if (/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?שעתיים/u.test(t)) return new Date(now.getTime() + 2 * 3_600_000).toISOString();
  const hrs = t.match(/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?(\d{1,2})\s*שע(?:ות|ה)?/u);
  if (hrs) return new Date(now.getTime() + Math.min(Number(hrs[1]), 48) * 3_600_000).toISOString();
  if (/(?:ב?עוד|לעוד)\s+(?:כ-?\s*)?שעה/u.test(t)) return new Date(now.getTime() + 3_600_000).toISOString();
...
export async function extractAlmogCommitments(params: {
  ...
  /**
   * האם מנוע ה-LLM רץ והחזיר תוצאה תקפה. אם כן — סומכים על *ההבנה* שלו (כולל
   * ההחלטה *לא* ליצור תזכורת על אזכור אגבי כמו "המורה הזכירה לי"), ולא מפעילים
   * את רשת הביטחון הדטרמיניסטית. רשת הביטחון היא רק ל-fallback כשה-LLM לא זמין.
   */
  let modelDecided = false;
  if (process.env.OPENROUTER_API_KEY?.trim()) {
    try {
      ...
      if (parsed) {
        extraction = normalize(parsed, now);
        modelDecided = true;
      }
    } catch {
  ...
  /**
   * רשת ביטחון דטרמיניסטית (בלי LLM) — *רק* כשה-LLM לא רץ/נכשל. אם המשתמש ביקש
   * מפורשות ואלמוג לא דחה, או שאלמוג הבטיח במפורש — יוצרים תזכורת בכל זאת, כדי
   * שבקשה לא תיעלם גם בלי OpenRouter. כשה-LLM כן רץ — מכבדים את החלטתו (גם []).
   */
  if (!modelDecided && extraction.reminders.length === 0) {
