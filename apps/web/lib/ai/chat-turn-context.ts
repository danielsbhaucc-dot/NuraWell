...
export function formatJourneyChatGuidanceBlock(opts: {
  ...
  const lines = [
    ...
    'habits: ○ = לא סומן — דרבן בעדינות לפעולה קטנה עכשיו.',
    ...
    'דרבון (חובה כשיש ○ פתוח): אל תסיים תשובה בלי להזמין לצעד אחד קונקרטי על הרגל/משימה ○ אחת — "בא נסגור את כוס המים?", "מתאים 10 דק׳ הליכה עכשיו?". בגודל שאי-אפשר לסרב לו, מתוך בחירה שלו, בלי אשמה ובלי רשימת חובות.',
  ];
  if (opts.isGreeting) {
    lines.push(
      'פתיחה (היי וכו\'): ברכה חמה קצרה ואז *מיד* דרבון קונקרטי — בחר הרגל ○ או משימה ○ אחת ספציפית (לא ✓) והזמן לפעולה קטנה עכשיו, או שאל אם כבר עשה אותה. לא סתם "מה שלומך?" בלי כיוון. בלי רשימת עובדות, בלי אשמה.'
    );
  }
  return lines.join('\n');
}
