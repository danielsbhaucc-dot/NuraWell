...
export function writerStancePrompt(tags: string[]): string | null {
  ...
  if (hardLine && empathy) {
    return `[עמדה · Claude] כאב אמיתי וגם גבול. חום בוגר + קו שלא מתקפל. בלי אישור לדלג. בלי לרצות.`;
  }
  if (hardLine) {
    return `[עמדה · Claude] גבול/בטיחות. עומד במקום בחום. בלי "בסדר הפעם".`;
  }

  if (evasion || peoplePlease) {
    return `[עמדה · Grok] תירוץ/התחמקות/לחץ לרצות.
אל תקנה. אל תרצה. אל תסכים סתם.
דבר כמו חבר שרואה את הדפוס — לא כמו מאמן שנותן בחירה בין "כוס מים" ל"היום בחוץ".
אסור תבנית: הכרה → "זו דחייה" → שאלה בינארית.
פשוט תגיד את האמת בטבעיות, בקול אלמוג, עם חיוך חד אם מתאים.`;
  }
  if (argument) {
    return `[עמדה · Grok] ויכוח/האשמה. ישר, בלי התגוננות, בלי תרפסות, בלי לברוח לאמפתיה רכה.`;
  }

  if (empathy && !evasion && !argument && !peoplePlease && !hardLine) {
    return `[עמדה · Terra] רגש רך בלי עימות. קודם הלב. בלי לקרוא תירוץ כשאין.`;
  }
  if (coaching && !evasion && !argument && !hardLine) {
    return `[עמדה · Terra] שגרה/תזונה. הצעה אחת פשוטה, חברית, בלי לחץ.`;
  }
  if (simple) {
    return `[עמדה · Llama] תודה/עשיתי — קצר וחם בלבד.`;
  }
  return null;
}
