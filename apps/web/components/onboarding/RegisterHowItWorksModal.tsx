  ...
  return (
    <AnimatePresence>
      {open ? (
        ...
        >
          <button type="button" className="absolute inset-0 bg-black/60 backdrop-blur-sm" aria-label="סגור" onClick={onClose} />
          <motion.div
            initial={{ y: 40, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 24, opacity: 0 }}
            className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border border-white/20 bg-slate-900/95 backdrop-blur-2xl p-5 sm:p-6"
          >
            <button
              type="button"
              onClick={onClose}
            ...
            <h2 id="how-it-works-title" className="text-xl font-black text-white mt-8 mb-4 text-right" style={{ fontFamily: 'Rubik, Heebo, sans-serif' }}>
              איך זה עובד?
            </h2>
