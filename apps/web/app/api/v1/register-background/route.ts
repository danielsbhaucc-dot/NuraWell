...
export async function GET() {
  ...
  const { data } = await (supabase as any)
    .from('site_settings')
    .select('register_background_key, register_background_credit')
    .eq('id', 1)
    .maybeSingle();

  const key =
    (data as { register_background_key?: string | null } | null)?.register_background_key ??
    REGISTER_BACKGROUND_OBJECT_KEY;
  const credit = (data as { register_background_credit?: unknown } | null)?.register_background_credit ?? null;
