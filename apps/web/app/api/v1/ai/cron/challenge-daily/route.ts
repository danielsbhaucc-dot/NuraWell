...
import { runChallengeDailyCron } from '@/lib/challenge/run-challenge-cron';
...
export async function POST(request: Request) {
  ...
  const idleSkip = await maybeReturnCronIdleSkip(request, admin, 'challenge-daily');
  if (idleSkip) return idleSkip;

  const url = new URL(request.url);
  const dryRun = url.searchParams.get('dryRun') === '1' || url.searchParams.get('dry_run') === '1';

  const result = await runChallengeDailyCron(admin, { dryRun });
  console.log('[challenge-daily CRON]', JSON.stringify(result));
