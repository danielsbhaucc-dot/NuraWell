...
export async function GET(request: Request) {
  ...
  const key = data?.register_background_key ?? REGISTER_BACKGROUND_OBJECT_KEY;
  const url = getPublicCdnImageUrl(key, String(Date.now()));

  return NextResponse.json({
    object_key: key,
    cover_url: url,
    credit: data?.register_background_credit ?? null,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const oldKey = settings?.register_background_key as string | undefined;
    if (oldKey && oldKey !== REGISTER_BACKGROUND_OBJECT_KEY) {
      try {
    ...
    for (const legacy of REGISTER_BACKGROUND_LEGACY_KEYS) {
      try {
    ...
    await s3.send(
      new PutObjectCommand({
        Bucket: bucket,
        Key: REGISTER_BACKGROUND_OBJECT_KEY,
        Body: buf,
    ...
    const { error } = await (supabase as any)
      ...
      .update({
        register_background_key: REGISTER_BACKGROUND_OBJECT_KEY,
        register_background_credit: creditParsed.data,
        updated_at: new Date().toISOString(),
      })
      .eq('id', 1);
    ...
    return NextResponse.json({
      ok: true,
      cover_url: getPublicCdnImageUrl(REGISTER_BACKGROUND_OBJECT_KEY, version),
      object_key: REGISTER_BACKGROUND_OBJECT_KEY,
      cdn_hostname: almogCdnHostname(),
    });
  } catch (e) {
    console.error('[register-background POST]', e);
...
export async function DELETE(request: Request) {
  ...
  const s3 = getR2Client();
  const keys = [REGISTER_BACKGROUND_OBJECT_KEY, ...REGISTER_BACKGROUND_LEGACY_KEYS];
  ...
  await (supabase as any)
    ...
    .update({
      ...
      register_background_credit: null,
      updated_at: new Date().toISOString(),
    })
    .eq('id', 1);
