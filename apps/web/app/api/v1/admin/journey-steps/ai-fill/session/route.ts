...
2. lesson_transcript — מה תמלול/תוכן השיעור, מה התלמיד מקבל. אם אין בטקסט תמלול/תוכן שיעור ברור (רק מחקרים או טקסט חלקי) — *בקש מפורשות מהמנהל את התמלול המלא של השיעור*, כי בלעדיו אי אפשר לבנות שאלות הבנה ונקודות קשב איכותיות.
3. user_goal — מה היעד ההתנהגותי, המשימה העיקרית, ההרגל, וסולם הקושי.

ניתוח פסיכולוגי-פדגוגי: על בסיס התמליל, חשוב כמה שאלות הבנה, טענות משחק ונקודות קשב באמת נכון לבנות לפי עושר התוכן ומספר הרעיונות המרכזיים. אם התמליל דליל מכדי לבנות תוכן איכותי — שאל על כך.

מטרת שאלות החידוד: לא "לשאול בשביל לשאול", אלא להשיג את המידע שחסר כדי למלא את הצעד
בצורה נכונה, איכותית ומדויקת. כל שאלה חייבת לשפר החלטה מעשית אחת במילוי הצעד:
- מה המסר המרכזי שהמשתמש אמור להבין.
- איזו פעולה התנהגותית אחת המשתמש אמור לבצע.
- איך לתרגם את הפעולה למשימה/הרגל מדידים וברורים.
- איך לבנות סולם קושי בטוח ולא מוגזם.
- איפה המחקר מוגבל, לא ודאי, או דורש ניסוח זהיר.
- מה חשוב לא להכניס לצעד כדי לא ליצור הבטחות שווא או עומס.
...
חוקים:
- בהדבקה ראשונה (כשעדיין אין "תשובות המנהל" בהודעה) — *חובה* להחזיר status "needs_clarification" עם 3-4 שאלות חידוד חכמות. אל תחזיר "ready" בהדבקה ראשונה גם אם הטקסט נראה מלא.
- שאל רק שאלות שהן high-leverage: התשובה אליהן תשנה בפועל את הכותרת, הסיכום, המשימה, ההרגל, סולם הקושי, המחקרים, או האזהרות.
- אל תשאל שאלות גנריות כמו "מה חסר?" או "יש עוד משהו?". שאל שאלה ספציפית שמצביעה על פער שזיהית בטקסט.
- העדף שאלות שמכריחות בחירה/דיוק: קהל יעד, תוצאה התנהגותית, מדד הצלחה, רמת קושי, גבולות המחקר.
- שאל 3-4 שאלות ממוקדות, לא יותר.
- כתוב בעברית.
- אל תמציא עובדות.
  ...
  if (action === 'start') {
    ...
    try {
      ...
      const fallbackQuestions = [
        {
          id: 'core-outcome',
          label: 'מה התוצאה המדויקת שהמשתמש אמור לצאת איתה מהצעד הזה — להבין משהו, לשנות בחירה, או לבצע פעולה יומית?',
          help_text:
            'נסה לנסח במשפט אחד: "אחרי הצעד המשתמש ידע/יעשה/יבחר ___". זה יכוון את הכותרת, הסיכום והמשימות.',
          input_type: 'textarea' as const,
          required: true,
        },
        {
          id: 'behavior-design',
          label: 'מה הפעולה ההתנהגותית האחת שהכי נכון להפוך למשימה או להרגל, ואיך נמדוד שהיא בוצעה?',
          help_text:
            'לדוגמה: "לשתות כוס מים לפני קפה", "10 דקות הליכה אחרי ארוחה", "לסמן רעב 1-10 לפני נשנוש".',
          input_type: 'textarea' as const,
          required: true,
        },
        {
          id: 'research-limits',
          label: 'מה המחקר באמת מאפשר להגיד, ומה אסור להבטיח או להציג כוודאות?',
          help_text:
            'ציין מגבלות, אוכלוסייה, חוזק ראיות, ניסוחים שצריך לרכך, או טענות שעדיף לא לכלול.',
          input_type: 'textarea' as const,
          required: false,
        },
        {
          id: 'difficulty-ladder',
          label: 'איך לבנות סולם קושי בטוח ובר ביצוע — קל / רגיל / מאתגר — בלי להעמיס על המשתמש?',
          help_text:
            'כתוב 2-3 רמות. המטרה היא שה-AI ימלא משימות מדויקות שמתאימות גם למשתמש שמתחיל חלש.',
          input_type: 'textarea' as const,
          required: false,
        },
      ];
